...
\begin{frame}[t]
	...
	\begin{columns}[t]
		...
		\begin{column}{\colwidthleft}
			...
			\begin{block}{What is Walk-on-Boundary?}

				A random walk that lives on the boundary: each hop samples a new boundary point from the Green's-function kernel, and the walk's weighted contributions estimate the boundary density $\mu$.

				\begin{figure}
					\centering
					\includegraphics[width=0.75\linewidth]{Images/Helmholtz WoB Poster-selection.png}
				\end{figure}

				\begin{itemize}
					\item Each hop $y_i \to y_{i+1}$ is one application of the boundary operator $A$ --- a single unbiased sample, no mesh anywhere.
					\item Classic WoB chains many hops to estimate $A^n$; my method takes \textbf{one hop per iteration} instead.
				\end{itemize}
				...
				\begin{figure}
					...
					\begin{tikzpicture}[
						...
						box/.style={draw, rounded corners, align=center, minimum height=2.3cm,
							minimum width=4.4cm, text width=4.1cm, font=\small, fill=white},
						...
						\draw[->, dashed] (res.north) .. controls +(0,3.6) and +(0,3.6) .. (shift.north);
